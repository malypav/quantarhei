...
with energy_units("1/cm"):
    ...
    mol2 = Molecule(elenergies=[0.0, 12500.0])
...
mju2=numpy.array([3.0,-1.0,0.0])
mju1=4*mju1/numpy.linalg.norm(mju1)
mju2=3.3*mju2/numpy.linalg.norm(mju2)

print('Transition dipoles:')
print(str(mju1)+'\n'+str(mju2))

mol1.set_dipole(0,1,mju1)
#mol2.set_dipole(0,1,[1.0, 2.0, 0.0])
mol2.set_dipole(0,1,mju2)
#
...
params = dict(ftype="OverdampedBrownian", T=300, reorg=50.0, cortime=100.0,matsubara=100)
with energy_units('1/cm'):
    cf = CorrelationFunction(tsbi, params)
    
params2 = dict(ftype="UnderdampedBrownian", T=77, reorg=10.0, freq=150.0,
               gamma=1.0/10000.0)
with energy_units('1/cm'):
...
tcalc = TwoDSpectrumCalculator(t1axis=ta, t2axis=t2s, t3axis=ta,
                               system=agg,twodtype="2DES",gamma_factor=1)
...
with energy_units("1/cm"):
    ...
    for tt2 in t2s.data:
        ...
        # plot it
       # label="2DES\nJ=29 cm"+r'$^{-1}$'+"\n",
        spect.plot(window=window_2D,label=str(tt2)+" fs",
                   text_loc=[0.03,0.9],fontsize="15",
                   zero_contour=False, Npos_contours=12) #,vmax=1.0, cbmax=cbmax)
#        spect.plot()
        # save figure
#        figname = "fig_2DES_"+str(round(tt2))+".png"
        figname = "fig_2DES_J0.5X.png"
...
#
##sp = twods.get_spectrum(t2s.data[0])
##with energy_units("1/cm"):
